"""Train the two-hidden-layer regression head 1024 -> 128 -> 64 -> 1 on SST
sentence embeddings.
...
class Mlp2Head(nn.Module):
    """1024 -> 128 -> 64 -> 1, GELU + dropout after each hidden layer, sigmoid to
    keep the output in [0, 1]."""

    def __init__(self, input_dim=1024, hidden1=128, hidden2=64, dropout=0.2):
        super().__init__()
...
def main():
    args = parse_args()
    model = Mlp2Head(dropout=args.dropout)
    print(f"model: {model}")
    train_head(
        model,
